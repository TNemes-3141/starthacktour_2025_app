...
class ObjectReferenceSizes {
  ...
  static const Set<String> _relevantLabels = {
    "person", "airplane", "bird", "kite", "umbrella", "surfboard", "unknown", // example subset
  };
  ...
  static bool isRelevantLabel(String label) {
    return _relevantLabels.contains(label);
  }
...
class CoordinateTransformer {
  ...
  static double estimateDistance(
    ...
    if (clampedDistance != estimatedDistance) {
      print(
        'Distance clamped for $objectType: ${estimatedDistance.toStringAsFixed(2)}m -> ${clampedDistance.toStringAsFixed(2)}m',
      );
    }
  ...
  static double estimateDistanceFromDiagonal(
    String objectType,
    ...
    final realSize = ObjectReferenceSizes.getReferenceSize(objectType);
    final avgFocalLength =
        (config.focalLengthPixelsHorizontal +
            config.focalLengthPixelsVertical) /
        2.0;
  ...
  static Map<String, double> pixelToAngles(
    ...
    final normalizedX =
        (pixelX - config.resolutionWidth / 2) / (config.resolutionWidth / 2);
    final normalizedY =
        (pixelY - config.resolutionHeight / 2) / (config.resolutionHeight / 2);
    ...
    final relativeElevationDeg =
        -normalizedY *
        (config.verticalFovDeg / 2); // Negative because Y increases downward

    // Calculate absolute angles
    final absoluteAzimuthDeg =
        (config.cameraAzimuthDeg + relativeAzimuthDeg) % 360;
    final absoluteElevationDeg =
        config.cameraElevationDeg + relativeElevationDeg;

    return {'azimuth': absoluteAzimuthDeg, 'elevation': absoluteElevationDeg};
  }
  ...
  static ObjectCoordinates transformToCoordinates({
    ...
    final deltaLongitude = _metersToLongitudeDegrees(
      eastDisplacement,
      cameraLatitude,
    );
...
extension TrackCoordinateExtension on Track {
  ...
  ObjectCoordinates calculateCoordinatesFromPixels({CameraConfig? config}) {
